  ...
  const { data: rdos, isLoading: rdosLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, obra_id, data, status, observacoes, condicoes_meteorologicas, fotos, trabalhos_executados, mao_de_obra_presente, ocorrencias, trabalhos_quantificados, aprovado_em, created_at, updated_at')
        .eq('obra_id', obraId)
