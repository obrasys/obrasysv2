...
import { ArrowLeft, Sparkles, RefreshCw, RotateCcw, Wand2, Upload as UploadIcon, Send } from "lucide-react";
...
export default function PlantaLeituraPage() {
  ...
  const { elements: sheetElements, refresh: refreshElements } = usePlantElements(activeFileId || undefined, activeSheetId || undefined);
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto w-full">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate(`/obras/${obraId}`)}>
          <ArrowLeft className="h-4 w-4 mr-1" /> Voltar à obra
        </Button>
        ...
        {files.length > 0 && (
          <select
            value={activeFileId || ""}
            onChange={(e) => { setActiveFileId(e.target.value); setActiveSheetId(null); }}
            ...
            {sheets.map((s) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition ${
                  s.id === activeSheetId ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-muted"
                }`}
              >
                Folha {s.sheet_index} · {s.discipline || "?"} · {s.status === "pending" ? "Por analisar" : s.status === "processing" ? "A analisar..." : s.status === "low_confidence" ? "Baixa confiança" : s.status === "error" ? "Erro" : "OK"}
              </button>
            ))}
          </div>
              ...
              <div className="p-4 border-b">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <Badge variant="outline" className="text-[10px] mt-1">IA + Humano</Badge>
                  </div>
                  {activeSheet?.confidence !== null && activeSheet?.confidence !== undefined && (
                    <div className="text-right">
                      <div className="text-xs text-muted-foreground">Confiança</div>
                      <div className="text-lg font-semibold text-primary">{Math.round((activeSheet.confidence || 0) * 100)}%</div>
                    </div>
                  )}
                </div>
                <div className="text-xs text-muted-foreground mt-2 space-y-0.5">
                  ...
                  <div>Folha {activeSheet?.sheet_index} · {activeSheet?.discipline || "—"} · {activeSheet?.floor_level || "—"}</div>
                </div>
                <div className="grid grid-cols-2 gap-2 mt-3">
                  <Button size="sm" variant="outline" onClick={handleApproveAll}>Aprovar todos</Button>
                  <Button size="sm" variant="outline" onClick={handleAutoFix} disabled={analyzing}>
                    <Wand2 className="h-3.5 w-3.5 mr-1" /> Auto-fix
                  </Button>
                  <label>
                    <input type="file" className="hidden" accept=".pdf,.dxf,.png,.jpg,.jpeg" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUpload(f); }} />
                    <Button asChild size="sm" variant="outline" className="w-full">
                      <span><UploadIcon className="h-3.5 w-3.5 mr-1" /> Substituir</span>
                    </Button>
                  </label>
                  <Button size="sm" variant="outline" onClick={() => activeSheetId && handleAnalyze(activeSheetId)} disabled={analyzing || !activeSheet?.image_path}>
                    <RefreshCw className="h-3.5 w-3.5 mr-1" /> {sheetElements.length === 0 ? "Analisar" : "Reprocessar"}
                  </Button>
              ...
              <Tabs defaultValue="elements" className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1 overflow-y-auto p-3">
                  <TabsContent value="elements" className="m-0">
                    <PlantElementsList elements={sheetElements} selectedId={selectedId} onSelect={setSelectedId} onChanged={refreshElements} />
                  </TabsContent>
