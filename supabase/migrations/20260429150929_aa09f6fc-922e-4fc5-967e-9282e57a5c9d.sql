-- Fase 2: View unificada de quantitativos da planta
...
SELECT
  ...
  'compartimento'::text,
  'pavimento'::text,
  COALESCE(r.nome, 'Compartimento')                       AS descricao,
  COALESCE(r.tipo_compartimento, 'habitacao')             AS categoria,
...
-- 2c) Cómodos — rodapé (perímetro em metros)
SELECT
  r.id,
  r.plan_import_id,
  pi.obra_id,
...
-- 3) Elementos colocados — agrupados por símbolo
SELECT
...
COMMENT ON VIEW public.plan_quantitativos_v IS
  'Fase 2 — Tabela unificada: agrega medições, cómodos (pavimento/teto/rodapé) e elementos colocados por pavimento com nível de confiança.';
