...
BEGIN
  ...
  -- Step 3: check equipa_membros OR subempreiteiros
  SELECT EXISTS(
    SELECT 1 FROM equipa_membros WHERE user_id = p_user_id
    UNION ALL
    SELECT 1 FROM subempreiteiros WHERE user_id = p_user_id
  ) INTO v_s3;
